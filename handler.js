            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@user', '@' + user.split('@')[0]).replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow')
            ...
            if (chat.bye) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (chat.sBye || this.bye || conn.bye || 'Bye, @user!').replace('@user', '@' + user.split('@')[0]).replace('@subject', await this.getName(id)).replace('@desc', await groupMetadata.desc?.toString() || 'unknown')
...
let msg = {
        ...
        premiumonly: `📢 Kamu tidak dapat mengakses fitur ❗\nBeli premium untuk mengakses lewat PC.\nhubungi owner dibawah\nwa.me/${global.info.nomerown}`,
        gconly: `📢 Kamu tidak dapat mengakses fitur ❗\nJoin group Nightmare Bot untuk mendapatkan akses\n\n> ${global.url.sgc}`,
  ...
  if (msg) return conn.sendMessage(m.chat, {
    ...
    let daftar = {
  unreg: `Untuk dapat mengakses seluruh fitur Bot,\nAnda harus melakukan \`\`\`DAFTAR\`\`\` terlebih dahulu.\nIntuk melakukan _daftar_ caranya cukup mudah\nketik: /daftar nama.umur\ncara lainnya:\n*ketik: @verify*`}[type]
